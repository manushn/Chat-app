...
router.post("/signup",async (req,res)=>{
    ...
    try{
        ...
        if(usernamefind&&emailfind){
            return res.status(203).json({emessage:"Username And Email Already Taken!"});
        }else if(emailfind){
            return res.status(203).json({emessage:"Email Already Taken!"})
        }else if(usernamefind){
            return res.status(203).json({emessage:"Username Already Taken!"})
        }else{
